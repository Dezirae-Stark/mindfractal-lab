...
class TimelineSlicer:
    ...
    def slice_parameter_line(self, start: ParameterPoint, end: ParameterPoint,
                           n_steps: int = 20) -> OrbitBranch:
        """
    ...
    def slice_random_walk(self, start: ParameterPoint, n_steps: int = 20,
                         step_size: float = 0.1) -> OrbitBranch:
        ...
        for _ in range(n_steps - 1):
            ...
            delta_c = step_size * (np.random.randn(self.manifold.dim) +
                                  1j * np.random.randn(self.manifold.dim))
    ...
    def _interpolate_points(self, p1: ParameterPoint, p2: ParameterPoint,
                          t: float) -> ParameterPoint:
