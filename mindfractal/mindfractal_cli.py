...
def cmd_visualize(args):
    ...
    if args.mode == "orbit":
        ...
        fig = plot_basin_of_attraction(
            model, resolution=args.resolution, save_path=args.output
        )
        print(f"Basin of attraction plot saved to {args.output}")

    elif args.mode == "lyapunov":
        fig = plot_lyapunov_spectrum(
            model, x0, n_steps=args.steps, save_path=args.output
        )
...
def main():
    ...
    sim_parser.add_argument(
        "--x0", nargs=2, type=float, help="Initial condition (x1 x2)"
    )
    ...
    viz_parser.add_argument(
        "--resolution", type=int, default=200, help="Grid resolution"
    )
    viz_parser.add_argument(
        "--output", type=str, required=True, help="Output image file"
    )

    # Fractal command
    frac_parser = subparsers.add_parser("fractal", help="Generate fractal map")
    frac_parser.add_argument(
        "--resolution", type=int, default=500, help="Grid resolution"
    )
    frac_parser.add_argument(
        "--c1-range",
        nargs=2,
    ...
    frac_parser.add_argument(
        "--max-steps", type=int, default=500, help="Max simulation steps"
    )
    frac_parser.add_argument(
        "--criterion",
    ...
    frac_parser.add_argument(
        "--save-data", type=str, help="Save raw data (NumPy .npy format)"
    )
    ...
    if TD_AVAILABLE:
        ...
        # td slice
        td_slice_parser = td_subparsers.add_parser(
            "slice", help="Create timeline slice"
        )
        td_slice_parser.add_argument(
            "--dim", type=int, default=2, help="Dimension (2 or 3)"
        )
        td_slice_parser.add_argument(
        ...
        td_slice_parser.add_argument(
            "--no-plot", action="store_true", help="Skip visualization"
        )
        td_slice_parser.add_argument(
        ...
        # td visualize
        td_viz_parser = td_subparsers.add_parser(
            "visualize", help="Visualize stability landscape"
        )
        td_viz_parser.add_argument("--dim", type=int, default=2, help="Dimension")
        td_viz_parser.add_argument(
            "--resolution", type=int, default=50, help="Grid resolution"
        )
        td_viz_parser.add_argument(
            "--min-param", type=float, default=-2.0, help="Min parameter"
        )
        td_viz_parser.add_argument(
            "--max-param", type=float, default=2.0, help="Max parameter"
        )
        ...
        # td random-orbit
        td_orbit_parser = td_subparsers.add_parser(
            "random-orbit", help="Generate random orbit"
        )
        td_orbit_parser.add_argument("--dim", type=int, default=2, help="Dimension")
        td_orbit_parser.add_argument(
            "--steps", type=int, default=500, help="Number of orbit steps"
        )
        ...
        # td boundary-map
        td_bound_parser = td_subparsers.add_parser(
            "boundary-map", help="Map stability boundaries"
        )
        td_bound_parser.add_argument("--dim", type=int, default=2, help="Dimension")
        td_bound_parser.add_argument(
            "--resolution", type=int, default=100, help="Grid resolution"
        )
        td_bound_parser.add_argument(
            "--min-param", type=float, default=-2.0, help="Min parameter"
        )
